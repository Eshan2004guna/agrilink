...
export const FarmerOrdersPage: React.FC = () => {
  ...
  const loadOrders = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await orderService.getOrders('FARMER', user.id);
      setOrders(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs">
        <h1 className="text-2xl font-extrabold text-slate-900">Received Customer Orders</h1>
        <p className="text-xs text-slate-500">Track incoming buyer requests, confirm status, and prepare harvest dispatch.</p>
      </div>
      ...
      {/* Orders Table */}
      {isLoading ? (
        <LoadingSpinner fullPage={false} message="Loading received orders..." />
      ) : filteredOrders.length === 0 ? (
        <EmptyState
          icon={<ShoppingBag className="w-10 h-10" />}
          title="No Orders Found"
          description={search ? "No orders match your filter criteria." : "You haven't received any customer orders yet."}
